...
export function ViewIndentDialog({
  ...
  useEffect(() => {
    if (open) {
      if (indent?.items) {
        setLocalItems(indent.items.map((i: any) => ({
          ...i,
          status: "Approved",
          workingQuantity: Number(i.quantity) || 1,
          isNew: false
        })));
  ...
  const canTakeAction = isAdmin
    ? indent.status === "ManagerApproved"
    : indent.status === "Pending";

  const handleApprove = () => {
    const approvedItems = localItems.filter(
      (item: any) => item.status === "Approved"
    );
    if (approvedItems.length === 0) {
      toast.error(
        "At least one item must be approved. To reject the whole indent, use the 'Reject Entire Indent' button."
      );
      return;
    }

    const formattedItems = approvedItems.map(
      (i: any) => {
        return {
          itemId: i.itemId?._id || i.itemId,
          unitId: i.unitId?._id || i.unitId,
          quantity: i.workingQuantity || Number(i.quantity),
        ...
        <div className="p-8 pb-4 bg-white shrink-0">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              {(indent.status === "Pending" || indent.status === "Rejected") && (
                <CreateIndentDialog
                  initialData={indent}
                  onSuccess={() => {
                    setOpen(false);
                  ...
                  trigger={
                    <Button
                      variant="outline"
                      className="h-9 px-4 rounded-full font-black text-[10px] gap-2 border-zinc-200 text-zinc-600 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50 shadow-sm"
                    >
                      <SquarePen className="h-3.5 w-3.5" /> Edit Indent
                    </Button>
                  }
                />
              )}
              <Badge
                className={cn(
                  ...
                  ["PENDING MANAGER", "PENDING ADMIN"].includes(currentLabel)
                    ? "bg-amber-100 text-amber-700"
                    : currentLabel === "APPROVED"
                      ? "bg-blue-100 text-blue-700"
                      : currentLabel === "PO CREATED"
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">
                    {indent.requestedBy?.email || "Requester"}{" "}
                    {indent.requestedBy?.mobile
                      ? `· ${indent.requestedBy.mobile}`
                      : ""}
                  </span>
                </div>
              ...
              {[
                {
                  label: "Project",
                  val:
                    indent.projectId?.projectName ||
                    indent.projectId?.name ||
                    "N/A",
                  icon: Building2
                },
                indent.outsideId
                  ? {
                      label: "Outside Area",
                      val:
                        indent.outsideId?.outsideName ||
                        indent.outsideId?.name ||
                        "N/A",
                      icon: MapPin
                    }
                  : {
                ...
                !indent.outsideId && {
                  ...
                  val:
                    [
                      indent.floorId?.floorName || indent.floorId?.name,
                      indent.flatId?.flatNumber ||
                        indent.flatId?.flatName ||
                        indent.flatId?.name
                    ]
                      .filter(Boolean)
                      .join(" · ") || "N/A",
                ...
                },
                {
                  ...
                  val: indent.storageLocation || "N/A",
                  icon: Layers
                },
                {
                  label: "Indent Type",
                  val: indent.indentType
                    ? indent.indentType.charAt(0).toUpperCase() +
                      indent.indentType.slice(1)
                    : "Item",
                  icon: Layers
                },
                {
                  label: "Priority",
                  val: indent.priority
                    ? indent.priority.charAt(0).toUpperCase() +
                      indent.priority.slice(1)
                    : "Medium",
                  icon: Zap
                }
              ]
                ...
                .map((item: any, i) => (
                  <div key={i} className="flex items-start gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[13px] font-black text-zinc-900 tracking-tight">
                        {item.val}
                      </span>
                    </div>
                  </div>
            ...
            {indent.rejectionReason && (
              <div className="p-5 bg-rose-50 rounded-2xl border border-rose-100 border-dashed">
                <span className="text-[8px] font-black text-rose-500 uppercase tracking-widest block mb-1">
                  Rejection Reason
                </span>
                <p className="text-[11px] font-bold text-rose-600 italic leading-relaxed">
                  &quot;{indent.rejectionReason}&quot;
                </p>
              </div>
            )}
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <h4 className="text-[10px] font-black text-zinc-900 uppercase tracking-widest">
                  {indent.indentType === "asset" ||
                  indent.indentType === "assets"
                    ? "Requested Assets"
                    : "Requested Items"}
                </h4>
              </div>
              ...
              {localItems.map((item: any, i: number) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between border-b border-zinc-100 pb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col flex-1 min-w-[200px]">
                        {item.isNew ? (
                           <Select
                             value={item.itemId?._id || item.itemId || ""}
                             onValueChange={(val) => {
                               const selectedItem = availableItems.find(a => a._id === val || a.id === val);
                               setLocalItems(prev => {
                                 const copy = [...prev];
                                 copy[i].itemId = selectedItem || val;
                                 return copy;
                               });
                             }}
                           >
                             <SelectTrigger className="h-8 text-sm font-black border-zinc-200">
                               <SelectValue placeholder="Select Item" />
                             </SelectTrigger>
                             <SelectContent>
                               {availableItems.map(a => (
                                 <SelectItem key={a._id || a.id} value={a._id || a.id}>{a.itemName || a.name}</SelectItem>
                               ))}
                             </SelectContent>
                           </Select>
                        ) : (
                      ...
                      {canTakeAction ? (
                        <div className="flex items-center gap-1 bg-zinc-50 p-1 rounded-xl border border-zinc-100 shrink-0">
                          ...
                          <Button
                            ...
                            className="h-7 w-7 p-0 text-rose-400 hover:text-rose-600 hover:bg-rose-100 transition-all rounded-lg ml-1"
                          >
                        ...
                        <Badge
                          variant="outline"
                          className="text-[7px] font-black uppercase px-2 py-0.5 border-zinc-100 rounded-md text-zinc-400"
                        >
                          {indent.priority || "low"}
                        </Badge>
                      )}
                      ...
                      {canTakeAction ? (
                        <div className="flex flex-col gap-1">
                          <Input
                            type="number"
                            min={1}
                            max={item.isNew ? undefined : Number(item.quantity)}
                            value={item.workingQuantity || ""}
                            onChange={(e) => {
                              const val = Number(e.target.value);
                              setLocalItems((prev) => {
                                const copy = [...prev];
                                copy[i].workingQuantity = val;
                                return copy;
                              });
                            }}
                            onBlur={(e) => {
                              ...
                              if (val < 1 || isNaN(val)) {
                                setLocalItems((prev) => {
                                  const copy = [...prev];
                                  copy[i].workingQuantity = 1;
                                  return copy;
                                });
                              } else if (!item.isNew && val > Number(item.quantity)) {
                                setLocalItems((prev) => {
                                  const copy = [...prev];
                                  copy[i].workingQuantity = Number(item.quantity);
                                  return copy;
                                });
                              }
                            }}
                          ...
                          />
                          {item.workingQuantity > 0 && !item.isNew &&
                            item.workingQuantity < Number(item.quantity) && (
                              <span className="text-[9px] text-amber-600 font-bold leading-tight">
                                {Number(item.quantity) - item.workingQuantity} will
                                automatically retry
                              </span>
                            )}
                        </div>
                        ...
                        <span className="text-xs font-bold text-zinc-900">
                          {item.workingQuantity || item.quantity}
                        </span>
                      )}
                    </div>
                    <div>
                      ...
                      {item.isNew ? (
                         <Select
                           value={item.unitId?._id || item.unitId || ""}
                           onValueChange={(val) => {
                             const selectedUnit = units.find(u => u._id === val || u.id === val);
                             setLocalItems(prev => {
                               const copy = [...prev];
                               copy[i].unitId = selectedUnit || val;
                               return copy;
                             });
                           }}
                         >
                           <SelectTrigger className="h-7 text-xs font-bold uppercase border-zinc-200">
                             <SelectValue placeholder="Unit" />
                           </SelectTrigger>
                           <SelectContent>
                             {units.map(u => (
                               <SelectItem key={u._id || u.id} value={u._id || u.id}>{u.label || u.value}</SelectItem>
                             ))}
                           </SelectContent>
                         </Select>
                      ) : (
                        <span className="text-xs font-bold text-zinc-900 uppercase">
                          {item.unitId?.label ||
                            item.unitId?.value ||
                            item.itemId?.unitId?.label ||
                            item.itemId?.unitId?.value ||
                    ...
                    <div className="col-span-1">
                      ...
                      {item.isNew ? (
                         <Input
                           ...
                           className="h-7 text-xs font-bold px-2 border-zinc-200"
                         />
                      ) : (
                        <span
                          className="text-xs font-bold text-zinc-600 italic block truncate"
                  ...
                  {(item.itemId?.groupId?.name ||
                    item.itemId?.subGroupId?.name ||
                    item.itemId?.price) && (
                      ...
                      {item.itemId?.price && (
                        <div>
                          ...
                          <span className="text-xs font-black text-zinc-900">
                            ₹{item.itemId.price} / unit{" "}
                            <span className="text-emerald-600 font-bold block text-sm">
                              Total: ₹
                              {Number(item.itemId.price) * item.quantity}
                            </span>
                          </span>
                        </div>
                      )}
                  ...
                  {item.images && item.images.length > 0 && (
                    <div className="space-y-2 border-t border-zinc-100/60 pt-3">
                      <span className="text-[8px] font-black text-zinc-400 uppercase tracking-widest block mb-1">
              ...
              {canTakeAction && (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setLocalItems(prev => [...prev, { status: "Approved", workingQuantity: 1, isNew: true }])}
                  className="w-full border-dashed border-zinc-300 text-zinc-500 hover:text-emerald-600 hover:border-emerald-300 hover:bg-emerald-50 rounded-2xl h-10 gap-2"
                >
          ...
          {canTakeAction && (
            <div className="w-full md:w-[380px] bg-emerald-50/10 border-t md:border-t-0 md:border-l border-emerald-100/60 p-8 flex flex-col justify-start shrink-0 space-y-5 overflow-y-auto custom-scrollbar">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-[10px] font-black text-emerald-950 uppercase tracking-widest">
                  Take Action
                </h4>
              </div>
...
export function CreateIndentDialog({
  ...
  const [locationTab, setLocationTab] = useState<"project" | "outside">(
    "project"
  );
  ...
  const fetchAssets = async (pageToFetch = 1, reset = false) => {
    ...
    try {
      ...
      setAvailableAssets((prev) =>
        reset ? newAssets : [...prev, ...newAssets]
      );
  ...
  const handleSubmit = async () => {
    ...
    if (items.some((i) => !i.itemId || !i.quantity || !i.unitId)) {
      toast.error(
        "Please select an item, quantity, and unit for all request items"
      );
      return;
    }
    ...
    if (estimateDeliveryDate) {
      formData.append(
        ...
        new Date(estimateDeliveryDate).toISOString()
      );
    }
    formData.append("indentType", indentType);
    formData.append(
      "itemType",
      indentType.charAt(0).toUpperCase() + indentType.slice(1)
    );
      ...
      <DialogContent className="sm:max-w-[750px] p-0 overflow-hidden border-none shadow-2xl rounded-[2.5rem] flex flex-col max-h-[95vh] mx-4">
        ...
        <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar bg-zinc-50/10">
          ...
          <div className="grid grid-cols-2 gap-8">
            <div className="space-y-3">
              <Label className="text-[11px] font-black text-zinc-900 uppercase tracking-tight">
                Project
              </Label>
              <Select
                value={projectId}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-3 items-center">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          document
                            .getElementById(`item-file-${item.id}`)
                            ?.click()
                        }
                      ...
                      {item.images?.map((img: any, imgIdx: number) => (
                        <div
                          key={imgIdx}
                          className="relative h-12 w-12 rounded-xl overflow-hidden border border-zinc-200 shadow-sm group"
                        >
                          <img
                            src={img.base64}
                            alt="Preview"
                              ...
                              setItems((prev) =>
                                prev.map((it, i) =>
                                  i === idx
                                    ? {
                                        ...it,
                                        images: it.images.filter(
                                          (_: any, k: number) => k !== imgIdx
                                        ),
                                        files: it.files
                                          ? it.files.filter(
                                              (_: any, k: number) =>
        ...
        <div className="p-10 pt-4 bg-white shrink-0">
          <Button
            ...
            className="w-full h-16 rounded-2xl bg-primary text-primary-foreground font-black text-lg shadow-2xl shadow-primary/20 hover:bg-primary/90 hover:scale-[1.01] transition-all transform active:scale-95"
          >
            Submit Request
          </Button>
        </div>
