...
export function CreatePODialog({ defaultIndentId, onSuccess, trigger }: CreatePODialogProps) {
  ...
  const handleIndentSelect = async (val: string) => {
    ...
    try {
      ...
      try {
        const poRes = await purchaseOrderService.getPurchaseOrders({ indentId: val, limit: 1000 })
        const activePOs = (poRes.data || []).filter((po: any) => po.status !== "Cancelled")
      ...
      if (fullIndent && Array.isArray(fullIndent.items)) {
         setItems(
            fullIndent.items.map((item: any) => {
               const itemIdStr = String(item.itemId?._id || item.itemId || "");
               const poCreated = item.poCreated || item.itemId?.poCreated || alreadyPoItemIds.has(itemIdStr);
               return {
                  itemId: itemIdStr,
  ...
  const subtotal = items.filter(item => item.selected).reduce((sum, item) => sum + ((Number(item.qty) || 0) * (Number(item.price) || 0)), 0)
                  ...
                  <div className="rounded-2xl border border-zinc-100 overflow-hidden">
                     <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-zinc-50/50 border-b border-zinc-100">
                          <th className="px-4 py-3 text-[9px] font-black text-zinc-400 uppercase tracking-widest w-[8%] text-center">
                            <input 
                              type="checkbox"
                              checked={items.length > 0 && items.filter(i => !i.poCreated).every(i => i.selected)}
                              onChange={(e) => {
                                const checked = e.target.checked;
                                setItems(prev => prev.map(item => item.poCreated ? item : { ...item, selected: checked }))
                              }}
                              className="rounded border-zinc-300 text-teal-600 focus:ring-teal-500 h-4 w-4 cursor-pointer"
                            />
                          </th>
                          <th className="px-6 py-3 text-[9px] font-black text-zinc-400 uppercase tracking-widest w-[27%]">Item</th>
                          <th className="px-6 py-3 text-[9px] font-black text-zinc-400 uppercase tracking-widest text-left w-[30%]">Description</th>
                          <th className="px-6 py-3 text-[9px] font-black text-zinc-400 uppercase tracking-widest text-center w-[15%]">Quantity</th>
                      ...
                      <tbody className="divide-y divide-zinc-50">
                        {items.map((item, idx) => (
                          <tr key={idx} className={item.poCreated ? "bg-zinc-50/50 opacity-70" : ""}>
                            ...
                            <td className="px-6 py-3 w-[27%]">
                              <div className="flex items-center gap-2">
                                ...
                                <div className="flex flex-col">
                                  ...
                                  {item.poCreated && (
                                    <span className="text-[9px] text-amber-600 font-semibold mt-1">PO is already created for this item</span>
                                  )}
                                </div>
                              </div>
                            </td>
                            <td className="px-6 py-3 w-[30%]">
                              <Input
                                placeholder={item.poCreated ? "PO already created" : "e.g. Specifications / Brand"}
                            ...
                            <td className="px-6 py-3 text-right text-xs font-bold text-zinc-900 w-[10%]">
                              ₹{(item.qty * (Number(item.price) || 0)).toLocaleString("en-IN")}
                            </td>
                          </tr>
                        ))}
                      </tbody>
