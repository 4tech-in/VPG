...
import { purchaseOrderService, PurchaseOrder } from "@/service/purchaseOrderService"
...
export function usePurchaseOrders() {
  ...
  const fetchPOs = useCallback(async () => {
    setIsLoading(true)
    try {
      const response = await purchaseOrderService.getPurchaseOrders({ page, limit, search })
  ...
  const approvePO = async (id: string, status: "Approved" | "Rejected", reason?: string) => {
    try {
      await purchaseOrderService.approvePurchaseOrder(id, { status, rejectionReason: reason })
