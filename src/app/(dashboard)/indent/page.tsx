...
export default function IndentPage() {
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex flex-col">
            <span className="font-bold text-zinc-900">
              {proj?.projectName || proj?.name || "N/A"}
            </span>
            <span className="text-[10px] text-zinc-400 font-medium">
              {outside?.outsideName ||
                outside?.name ||
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const preview = items
          .map((i: any) => i.itemId?.itemName || i.itemId?.name || "Unknown")
          .join(", ");
        return (
          <div className="flex flex-col">
            <span className="font-bold text-zinc-900">{count} Items</span>
            <span
              className="text-[10px] text-zinc-400 font-medium max-w-[200px] truncate"
              title={preview}
            >
              {preview}
            </span>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              >
                ...
                <SelectContent className="rounded-2xl border-zinc-100 shadow-2xl p-1">
                  <SelectItem
                    value="Approved"
                    className="rounded-xl font-bold text-[10px] uppercase tracking-wider py-3 text-emerald-600"
                  >
                    Approve
                  </SelectItem>
                  <SelectItem
                    value="Rejected"
                    className="rounded-xl font-bold text-[10px] uppercase tracking-wider py-3 text-rose-500"
                  >
                    Reject
                  </SelectItem>
                </SelectContent>
  ...
  return (
    <ContentLayout title="Indent Requests">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <CreateIndentDialog
              ...
              trigger={
                <Button className="h-12 px-6 rounded-2xl bg-primary text-primary-foreground font-black shadow-lg shadow-primary/20 gap-2 hover:bg-primary/90 hover:scale-[1.01] transition-all">
      ...
      <Dialog
        open={!!approvingIndent}
        onOpenChange={(open) => {
          if (!open) handleCancelApprove();
        }}
      >
        <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden border-none shadow-2xl rounded-[2.5rem] flex flex-col mx-4 bg-white max-h-[90vh]">
          <div className="p-8 pb-4 shrink-0 bg-white">
            ...
            <DialogDescription className="text-zinc-400 text-xs font-bold uppercase tracking-wider leading-relaxed">
              Please verify items list, adjust quantities, approve/reject items
              individually, and provide a remark.
            </DialogDescription>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-8 pt-4 space-y-6 bg-zinc-50/10 custom-scrollbar">
            ...
            <div className="space-y-4">
              <Label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">
                Items List (Approve / Reject & Adjust)
              </Label>
                          ...
                          {item.images.map((img: any, imgIdx: number) => {
                            ...
                            return (
                              <div
                                ...
                                title={
                                  img.fileName || `Attachment ${imgIdx + 1}`
                                }
            ...
            {approvingIndent?.images && approvingIndent.images.length > 0 && (
              <div className="space-y-3 pt-2">
                <Label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest block">
                  Attached Images ({approvingIndent.images.length})
                </Label>
            ...
            <div className="space-y-2">
              <Label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">
                Approval Description / Remark (Optional)
              </Label>
              <Textarea
